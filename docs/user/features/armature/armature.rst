...
* This is a special type of bone which is used to position an object in the
  inventory display.
...
#. Create this item in the same manner as you would for a standard armature
   bone.
#. Parent must be Armature root.
#. Naming must start with InvMarker and can only be appended with ``.000``

   A model with 4 inventory marker bone items should be named as ``InvMarker``,
   ``InvMarker.001``, ``InvMarker.002``, ``InvMarker.003``
